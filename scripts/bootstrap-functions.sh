...
_create_rbac_helpers() {
    ...
    local OKD_RBAC_VALUES_FILE=${OKD_VERSION:+"-f ${EL_CICD_DIR}/${EL_CICD_CHART_DEPLOY_DIR}/el-cicd-okd-scc-nonroot-builder-values.yaml"}
    ...
    helm upgrade --atomic --install --history-max=1 \
        ${SET_PROFILES} ${OKD_RBAC_VALUES_FILE} -f ${EL_CICD_DIR}/${EL_CICD_CHART_DEPLOY_DIR}/el-cicd-cluster-rbac-values.yaml \
        -n kube-system \
...
__create_onboarding_automation_server() {
    ...
    helm upgrade --atomic --install --history-max=1 \
        ...
        -f ${EL_CICD_DIR}/${EL_CICD_CHART_DEPLOY_DIR}/el-cicd-master-pipelines-values.yaml \
        -f ${EL_CICD_DIR}/${EL_CICD_CHART_DEPLOY_DIR}/jenkins-config-values.yaml \
        jenkins \
        elCicdCharts/elCicdChart
    set +ex
    ...
    helm upgrade --wait --wait-for-jobs --install --history-max=1  \
                ...
                -n ${EL_CICD_MASTER_NAMESPACE} \
                -f ${EL_CICD_DIR}/${EL_CICD_CHART_DEPLOY_DIR}/jenkins-pipeline-sync-job-values.yaml \
