...
def pushElCicdCredentialsToCicdServer(def projectInfo, def ENVS) {
    ...
    withCredentials([sshUserPrivateKey(credentialsId: keyId, variable: 'EL_CICD_GIT_REPO_READ_ONLY_GITHUB_PRIVATE_KEY_ID')]) {
    ...
    loggingUtils.echoBanner("PUSH ${keyId} CREDENTIALS TO CICD SERVER")
    withCredentials([sshUserPrivateKey(credentialsId: keyId, variable: 'EL_CICD_CONFIG_GIT_REPO_READ_ONLY_GITHUB_PRIVATE_KEY_ID')]) {
