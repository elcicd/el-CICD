...
__create_nexus_nfs_share() {
    NEXUS_DATA_PV_DIR=/mnt/nexus-data

    echo
    if [[ ! -d /mnt/nexus-data || -z $(sudo exportfs | grep ${NEXUS_DATA_PV_DIR}) ]]
    then
        echo 'Creating NFS share on host for Nexus, if necessary: /mnt/nexus-data'
        sudo mkdir -p ${NEXUS_DATA_PV_DIR}

        if [[ -z $(cat /etc/exports | grep ${NEXUS_DATA_PV_DIR}) ]]
        then
            echo "${NEXUS_DATA_PV_DIR} *(rw,sync,all_squash,insecure)" | sudo tee -a /etc/exports
        fi

        sudo chown -R nobody:nobody ${NEXUS_DATA_PV_DIR}
        sudo chmod 777 ${NEXUS_DATA_PV_DIR}
        sudo exportfs -a
...
__setup_nexus3() {
    ...
    then
        ...
        local NEXUS_K8S_FILE=nexus-local.yml
    fi
    echo
    ...
    __create_nexus_image_repositories
}
...
__remove_nexus_nfs_share() {
    echo
    echo 'Removing /mnt/nexus-data and delisting it as an NFS share'

    NEXUS_DATA_PV_DIR=/mnt/nexus-data
    sudo rm -rf ${NEXUS_DATA_PV_DIR}
    sudo sed -i "\|${NEXUS_DATA_PV_DIR}|d" /etc/exports
    sudo exportfs -a
    sudo systemctl restart nfs-server.service
}
