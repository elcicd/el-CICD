...
EL_CICD_UTILS_GITHUB_URL=${EL_CICD_GITHUB_URL}/el-CICD-utils/keys
...
echo
echo "Deleting read only deploy key for el-CICD-utils"
KEY_ID=$(curl -ksS -X GET ${EL_CICD_UTILS_GITHUB_URL} | jq ".[] | select(.title  == \"${EL_CICD_UTILS_SSH_READ_ONLY_PUBLIC_DEPLOY_KEY_TITLE}\") | .id")
curl -ksS -X DELETE ${EL_CICD_UTILS_GITHUB_URL}/${KEY_ID}
...
echo
echo "Adding read only deploy key for el-CICD-utils"
cat ${TEMPLATES_DIR}/githubSshCredentials-prefix.json | sed "s/%DEPLOY_KEY_NAME%/${EL_CICD_UTILS_SSH_READ_ONLY_PUBLIC_DEPLOY_KEY_TITLE}/" > ${SECRET_FILE}
cat ${EL_CICD_UTILS_SSH_READ_ONLY_DEPLOY_KEY_FILE}.pub >> ${SECRET_FILE}
cat ${TEMPLATES_DIR}/githubSshCredentials-postfix.json >> ${SECRET_FILE}
sed -i -e 's/false/true/' ${SECRET_FILE}

curl -ksS -X POST -H Accept:application/vnd.github.v3+json -d @${SECRET_FILE} ${EL_CICD_UTILS_GITHUB_URL}
